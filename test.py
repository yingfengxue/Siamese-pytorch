...
def evaluate_results(final_predictions, gt_base_dir, num_classes=3):
    """
    评估函数：计算 TP/FP/FN/Precision/Recall/F1，并计算 mAP（兼容旧版 mean_average_precision）。
    """
    preds_list = []
    gt_list = []
    ...
    for match_key, data in final_predictions.items():
        ...
        for gt in gt_boxes:
            ...
            # GT shape = [class_id, x1, y1, x2, y2, difficult]
            gt_list.append([int(gt[0]), x1, y1, x2, y2, 0])
        ...
        if cc_prop is not None and pred_class < 3 and pred_box_coords is not None:
            ...
            confidence_score = data.get('best_score', 1.0)
            # Pred shape = [class_id, confidence, x1, y1, x2, y2]
            preds_list.append([int(pred_class), confidence_score, x1, y1, x2, y2])
